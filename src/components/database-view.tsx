...
export function DatabaseView() {
  ...
  const fetchBinStatusData = async () => {
    console.log('🔄 Fetching bin_daily_data...');
    setIsLoadingBinStatus(true);
    try {
      const { data, error } = await supabase
        .from('bin_daily_data')  // ← CHANGED FROM BinStatus
        .select('*')
        .order('created_at', { ascending: false });

      if (error) {
        console.error('❌ Error fetching bin_daily_data:', error);
        console.error('Error details:', error.message, error.details, error.hint);
      } else if (data) {
        console.log('✅ bin_daily_data fetched:', data.length, 'records');
      ...
      console.error('💥 Exception fetching bin_daily_data:', err);
    } finally {
  ...
  const getBinStatusBadge = (status: string | null) => {
    if (!status) {
  ...
  const getFilteredAndSortedRecords = () => {
    ...
    if (statusFilter !== 'all') {
      filtered = filtered.filter(record => 
        record.BinStatus && record.BinStatus.toUpperCase() === statusFilter.toUpperCase()
      );
    }
    ...
    if (searchQuery) {
      filtered = filtered.filter(record =>
        (record.BinId && record.BinId.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (record.User_id && record.User_id.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (record.BinVersion && record.BinVersion.toLowerCase().includes(searchQuery.toLowerCase()))
      );
    }
  ...
  const stats = {
    ...
    active: binStatusRecords.filter(r => r.BinStatus && r.BinStatus.toUpperCase() === 'ACTIVE').length,
    full: binStatusRecords.filter(r => r.BinStatus && r.BinStatus.toUpperCase() === 'FULL').length,
    critical: binStatusRecords.filter(r => r.Total_fill_level >= 90).length,
    avgFill: binStatusRecords.length > 0 
      ? Math.round(binStatusRecords.reduce((sum, r) => sum + (r.Total_fill_level || 0), 0) / binStatusRecords.length)
      : 0
  };
